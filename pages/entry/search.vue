...
const fetchData = async (val?: number) => {
  ...
  if (val) {
    ...
    navigateTo({
      path: '/entry/search',
      query: { search: searchText.value, page: val },
    });
    window.scroll(0, 0);
  }

  const params = {
    toDate: searchStore.yearFilter
      ? searchStore.yearFilter + '-12-31T00:00:00.000Z'
      : '',
    fromDate: searchStore.yearFilter
      ? searchStore.yearFilter + '-01-01T00:00:00.000Z'
      : '',
      ...
      <Transition name="nested" duration="550">
        <div class="filters" v-if="isFilters">
          <div class="filters__header">
            ...
            <el-button class="filters__clear" @click="searchStore.clearFilter"
              >Очистить</el-button
            >
          </div>
